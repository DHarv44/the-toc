...
export default function PackOrg({ p, ed }: { p: Pack; ed: ManifestEditor }) {
  const [section, setSection] = useState<'templates' | 'rosters'>('templates')
  ...
  return (
    <Box maw={900}>
      <Group gap={4} mb={14}>
        {(['templates', 'rosters'] as const).map(s => (
          <Button key={s} size="compact-xs" variant={section === s ? 'filled' : 'default'}
            onClick={() => setSection(s)}>
            {s === 'templates' ? `TEMPLATES (${Object.keys(kinds).length})` : `ROSTERS (${rosterKeys.length})`}
          </Button>
        ))}
      </Group>
