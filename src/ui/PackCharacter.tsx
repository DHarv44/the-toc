    ...
    <Box maw={900}>
      ...
      {section === 'ranks' && (
        <>
          ...
          {ownsRanks && (
            <>
              <Text fz={15} c="dark.3" mb={10} maw={620}>
              ...
              {ranks.map((r, i) => {
                ...
                return (
                  <Group key={i} gap={6} mb={4} wrap="nowrap" align="center" p={6} style={CARD}>
                    <Text fz={14} c="dark.3" w={22} ta="right">{i + 1}</Text>
                    <Box w={26}><RankIcon rank={r.key} /></Box>
                    <TextInput size="sm" w={80} value={r.key} styles={{ input: { fontFamily: MONO } }}
                      onChange={e => setRanks(ranks.map((x, j) => j === i ? { ...x, key: e.currentTarget.value.toUpperCase() } : x))} />
                    <TextInput size="sm" style={{ flex: 1 }} value={r.name}
                      onChange={e => setRanks(ranks.map((x, j) => j === i ? { ...x, name: e.currentTarget.value } : x))} />
                    <NumberInput size="sm" w={58} min={0} max={4} value={ch[0]} title="chevrons"
                      onChange={v => setIns(i, { chevrons: [Number(v) || 0, ch[1]] })} />
                    <NumberInput size="sm" w={58} min={0} max={4} value={ch[1]} title="rockers"
                      onChange={v => setIns(i, { chevrons: [ch[0], Number(v) || 0] })} />
                    <NumberInput size="sm" w={54} min={0} max={5} value={ins.bars ?? 0} title="officer bars"
                      onChange={v => setIns(i, { bars: Number(v) || undefined })} />
                    <NumberInput size="sm" w={54} min={0} max={5} value={ins.stars ?? 0} title="general stars"
                      onChange={v => setIns(i, { stars: Number(v) || undefined })} />
                    <Group gap={2} wrap="nowrap">
                      <Button size="compact-sm" variant="subtle" disabled={i === 0}
                        onClick={() => move(i, -1)}>▲</Button>
                      <Button size="compact-sm" variant="subtle" disabled={i === ranks.length - 1}
                        onClick={() => move(i, 1)}>▼</Button>
                ...
                return (
                  <Box key={k} mb={6} p={8} style={CARD}>
                    <Group gap={8} wrap="nowrap" align="flex-end">
                      ...
                      <TextInput size="sm" w={90} value={a.abbr ?? ''} placeholder="PH"
                        styles={{ input: { fontFamily: MONO } }}
                        onChange={e => write({ abbr: e.currentTarget.value })} />
                      <TextInput size="sm" style={{ flex: 1 }} value={a.name ?? ''}
                        onChange={e => write({ name: e.currentTarget.value })} />
                      <TextInput size="sm" w={110} value={a.on ?? ''} placeholder="on (wound)"
                        styles={{ input: { fontFamily: MONO } }}
                        onChange={e => write({ on: e.currentTarget.value || undefined })} />
                      <Button size="compact-sm" variant="subtle" color="red"
                        onClick={() => { const { [k]: _g, ...rest } = awards; setAwards(rest) }}>✕</Button>
                    </Group>
                    <TextInput size="sm" mt={4} value={(a.ribbon ?? []).join(', ')}
      ...
      {section === 'callsigns' && (
        <Box maw={620}>
          ...
          <Text fz={15} fw={700} c="#9ab8d0" mb={4} style={{ letterSpacing: 1.5 }}>POOL</Text>
          <TextInput size="sm" mb={12} value={(cs.pool ?? []).join(', ')}
          ...
          <Group gap={10} align="flex-end">
            <TextInput size="sm" w={110} label="PREFIX" value={cs.prefix ?? ''} placeholder="E"
              styles={{ input: { fontFamily: MONO } }}
              onChange={e => ed.set('callsigns', { ...cs, prefix: e.currentTarget.value || undefined })} />
            <NumberInput size="sm" w={90} label="PAD" min={0} max={4} value={cs.pad ?? 0}
          ...
          <Switch mt={14} size="sm" checked={!!cs.pool?.length}
